...
class TSWriter {
    ...
    var server:ServerConnector = ServerConnector(address:"")
    ...
    fileprivate var senddata = Data()
    func setAddress(address:String){
        self.server.setAddress(address: address)
    }
    ...
    func writeSampleBuffer(_ PID:UInt16, streamID:UInt8, sampleBuffer:CMSampleBuffer) {
        ...
        nstry({
            ...
            self.senddata += Data(bytes: UnsafePointer<UInt8>(bytes),count: bytes.count)
        }){ exception in
            self.currentFileHandle?.write(Data(bytes: UnsafePointer<UInt8>(bytes), count: bytes.count))
            self.senddata += Data(bytes: UnsafePointer<UInt8>(bytes),count: bytes.count)
    ...
    func rotateFileHandle(_ timestamp:CMTime) -> Bool {
        ...
        nstry({
            ...
            let data = self.playlist.data(using: String.Encoding.utf8)
            self.server.sendFiles(tsData: data! as NSData, filename: "playlist.m3u8")
            self.senddata += Data(bytes: bytes,count: bytes.count)
            self.server.sendFiles(tsData: self.senddata as NSData, filename: filename)
        }){ exception in
            logger.warning("\(exception)")
        }
